...
func InitRouter() {
	...
	*/
	...
	auth.Use(middleware.JwtToken())
	{
		...
		// 附件模块路由
		auth.POST("attachment/upload", v1.UploadAttachment)
		auth.POST("attachment/batch-upload", v1.BatchUploadAttachments)
		auth.DELETE("attachment/:id", v1.DeleteAttachment)
	}
	...
	*/
	router := r.Group("api/v1")
	{
		...
		router.GET("/categories-with-articles/:id", v1.GetCategoriesWithTopArticles)
		...
		//收藏模块
		router.POST("addbookmark", v1.Addbookmark)
		router.DELETE("delbookmark", v1.DeleteBookmark)
		router.GET("bookmarklist", v1.GetBookmarkListFront)
		router.GET("bookmarkedarticles", v1.GetBookmarkedArticles)
		router.GET("checkbookmarked", v1.CheckBookMark)

		// 附件模块 - 前端可访问
		router.GET("attachment/list/:id", v1.GetAttachments) // 移动到这里，使前端可访问
		router.GET("attachment/download/:id", v1.DownloadAttachment)
	}
