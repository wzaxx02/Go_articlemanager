...
type Article struct {
	...
	Attachments  []Attachment `gorm:"foreignKey:ArticleID" json:"attachments"` // 添加这一行
}

type BookMarkedArticleV0 struct {
	Article
...
func GetArtInfo(id int) (Article, int) {
	var art Article
	err = db.Where("id = ?", id).Preload("Category").Preload("Attachments").First(&art).Error
...
// GetBookmarkedArticles 查询用户收藏的文章列表
func GetBookmarkedArticles(userId int, pageSize int, pageNum int) ([]BookMarkedArticleV0, int, int64) {
	var bookMarkedArticles []BookMarkedArticleV0
	var total int64

	// 查询用户收藏的文章，并预加载 Category
	query := db.Model(&Article{}).
		...
		`).
		Joins("INNER JOIN bookmark ON article.id = bookmark.article_id").
		Joins("INNER JOIN category ON article.cid = category.id").
		Where("bookmark.user_id = ? AND bookmark.deleted_at IS NULL", userId).
		Order("article.created_at DESC").
		Preload("Category") // 预加载 Category，确保过滤软删除

	err := query.Limit(pageSize).Offset((pageNum - 1) * pageSize).Find(&bookMarkedArticles).Error
	if err != nil {
		fmt.Println(err)
		return nil, errmsg.ERROR, 0
	}
	...
	return bookMarkedArticles, errmsg.SUCCESS, total
}
